(function(root) {
	...
	function releasePrintSources() {
		...
		printSourceUrls = [];
		printCards.forEach(function(card) { delete card.printSource; if (card.backFace) delete card.backFace.printSource; });
	}
	...
	async function openPrint(scope) {
		...
		try {
			...
			printCards.forEach(function(card) {
				var frontBlob = images[card.id];
				if (frontBlob) { var frontUrl = URL.createObjectURL(frontBlob); printSourceUrls.push(frontUrl); card.printSource = frontUrl; }
				var backBlob = images[card.id + ':back'];
				if (backBlob && card.backFace) { var backUrl = URL.createObjectURL(backBlob); printSourceUrls.push(backUrl); card.backFace.printSource = backUrl; }
			});
			renderPrintPages();
	...
	function renderPrintPages() {
		...
		document.querySelector('#desktop-print-pages').innerHTML = pages.map(function(cards, index) {
			...
			var back = '<section class="desktop-print-page desktop-print-page-back paper-' + selectedPaper + '" aria-label="Back print page ' + (index + 1) + '"><div class="desktop-print-grid">' + mirrored.map(function(card) { var source = root.SetConjurerPrint.backSource(card, '/core/standard-card-back.png'); return card ? '<article class="desktop-print-slot"><img src="' + source + '" alt="' + (card.backFace && card.backFace.printSource ? 'Custom reverse face' : 'Standard card back') + '"></article>' : '<article class="desktop-print-slot" aria-hidden="true"></article>'; }).join('') + '</div></section>';
			return front + back;
		}).join('');
	}
