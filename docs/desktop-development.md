...
Pull requests begin with a blobless, non-cone sparse checkout and classify the changed files through `packs/config.json`. Application jobs materialize source plus the four renderer-global frame assets; set-symbol and frame-image payloads remain absent. The keyword catalog remains available to the verification job so the complete recognition suite can run, but Forge excludes it from the application installer and the Keywords archive owns it at runtime. Pack-owned changes run metadata validation for the affected logical packs, while physical payload validation materializes only those packs. Shared ownership inputs deliberately expand validation to every pack. `npm run ci:classify -- --base <sha> --head <sha>` prints the same routing decision locally, and `npm run ci:checkout-report -- --expect-sparse` enforces the application checkout boundary.
