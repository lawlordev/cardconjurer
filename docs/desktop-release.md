...
The first selective schema-v3 release may set `bootstrap_v3_from_v2` only when a prior schema-2 catalog exists and no schema-3 catalog has ever been published. The bootstrap carries every unchanged immutable schema-2 archive record into the complete v3 snapshot, marks those histories with `legacySource`, and leaves their unavailable manifest digests unset; newly selected packs still receive schema-3 manifests and digests. After the first v3 catalog is published, every selective release must supply both previous catalog URLs and must not use the bootstrap flag.
